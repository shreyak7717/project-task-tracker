...
def test_field_edits_record_old_and_new(client, manager, auth_headers, make_project):
    ...
    client.patch(f"/api/tasks/{task['id']}", json={"title": "New"}, headers=auth_headers(manager))
    changed = [
        e
        for e in _timeline(client, auth_headers(manager), task["id"])
...
def test_status_change_and_comment_land_in_chronological_order(
...
def test_dependency_add_and_remove_are_recorded(client, manager, auth_headers, make_project):
    ...
    client.delete(f"/api/tasks/{b['id']}/dependencies/{a['id']}", headers=auth_headers(manager))
...
def test_there_is_no_route_to_edit_or_delete_a_timeline_entry(
    client, manager, auth_headers, make_project
):
    project = make_project()
    task = _task(client, auth_headers(manager), project.id)
...
def test_a_member_can_read_the_timeline_of_a_task_in_their_project(
    ...
    task = _task(client, auth_headers(manager), project.id)
    assert (
        client.get(f"/api/tasks/{task['id']}/timeline", headers=auth_headers(member)).status_code
        == 200
    )
